...
> 以下例子主要使用了返回主页 冷枚
在csdn上面的例子：[mybatis 全查 分页 模糊查询一体](https://www.cnblogs.com/woshuyuqiang/p/9742202.html)

1.不再赘述环境的搭建，可以从文章底部获取Github地址，进行搭建。

2.在CurdController中编辑如下：
...
6.创建一个vo包，在其下创建PageVo.java,代码如下：
...
<mapper namespace="com.chanchifeng.module.user.mapper.SysUserMapper">
  ...
  <!-- 全查 分页 模糊-->
  <select id="selectAll" resultMap="BaseResultMap" parameterType="com.chanchifeng.module.user.pojo.SysUser">
  ...
  <!--统计总数-->
  <select id="count" resultType="int" parameterType="com.chanchifeng.module.user.pojo.SysUser">
    select count(*)
    from sys_user
    <where>
      <if test=" sysUser.sysUserLoginName != null and sysUser.sysUserLoginName != '' ">
...
10.在浏览器中输入http://localhost:8080/curd/curdController/queryAll，效果图如下：

![](springmvc-spring-mybatis-pager/1.png)

### 上传文件

a.在controller包下创建一个UploadController，代码如下：
...
b.在WEB-INF/view下创建upload，upload文件夹下创建index.jsp,代码如下：
...
> github：[https://github.com/porschan/SpringMVC-Spring-MyBatis/curd](https://github.com/porschan/SpringMVC-Spring-MyBatis)